...
public class UserController {
    ...
    public ResponseEntity create(@RequestBody UserModel userModel){
        ...
        if (userModel.getUsername() == null || userModel.getUsername().isBlank()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Username obrigatorio");
        }

        if (userModel.getPassword() == null || userModel.getPassword().isBlank()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Password obrigatorio");
        }
